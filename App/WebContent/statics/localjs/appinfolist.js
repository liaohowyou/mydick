...
$(".addVersion").on("click", function() {
	var obj = $(this);
	window.location.href = "appversionadd.do?appId=" + obj.attr("appinfoid");
});
$(".modifyVersion").on("click",
		function() {
			var obj = $(this);
			...
			if (status == "1" || status == "3") {// 待审核、审核未通过状态下才可以进行修改操作
				if (versionid == null || versionid == "") {
					...
					window.location.href = "appversionmodify.do?vid="
...
var saleSwitchAjax = function(appId, obj) {
	$
			.ajax({
				type : "PUT",
				url : appId + "/sale"+obj.attr("saleSwitch")+".do",
				dataType : "json",
...
$(".viewApp").on("click", function() {
	var obj = $(this);
	window.location.href = "appview.do?id=" + obj.attr("appinfoid");
});
